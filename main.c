      ...
      for (count1 = 0; count1 < 8; count1++)
      {
        ...
        if (voltage[map[BAT]] == 0.0)    // No BAT Board
          ...
          else
          {
            ...
            if (sim_mode && !sim_config)
            { // if Voltage sensor on Battery board is present, exit simulated telemetry mode
              sim_mode = FALSE;
    ...
    */
    ...
    { // APRS
      ...
      if (c2cStatus == 0)
        ...
        sprintf(tlm_str, "BAT %4.2f %5.1f C VOL %4.2f %4.2f %4.2f %4.2f %4.2f %4.2f %4.2f ", voltage[map[BAT]], current[map[BAT]], voltage[map[PLUS_X]], voltage[map[MINUS_X]], voltage[map[PLUS_Y]], voltage[map[MINUS_Y]],
                voltage[map[PLUS_Z]], voltage[map[MINUS_Z]], voltage[map[BAT]]);
...
{ // returns 2 decimal point random number
  ...
  while ((!finished) && ((millis() - time_start) < CAMERA_TIMEOUT))
  {
    ...
    if (serialDataAvail(uart_fd))
    {
      ...
      if (start_flag_complete)
      {
        ...
        if (octet == end_flag[flag_count])
        { // looking for end flag
      ...
      else if (octet == start_flag[flag_count])
      { // looking for start flag
...
void get_tlm_fc()
{ // FUNcube Mode telemetry generation
...
#define FC_SIM
  ...
  unsigned int total_solar_current = (unsigned int)(current[map[PLUS_X]] + current[map[MINUS_X]] + current[map[PLUS_Y]] + current[map[MINUS_Y]] + current[map[PLUS_Z]] + current[map[MINUS_Z]]);
